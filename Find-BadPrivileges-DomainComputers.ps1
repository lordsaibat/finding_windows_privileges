...
function Find-BadPrivileges{
 <#
  ...
  .EXAMPLE
    ...
    Find-BadPrivileges -Computer TEST
...
Write-Host ""
Write-Host "Accounts with potentially Bad Privilege on $ComputerName" -ForegroundColor DarkYellow
...
if ($Accounts){
 Write-Host "Accounts with " $priv -ForegroundColor DarkYellow
 foreach ($Account in $Accounts){
 $Name=$Account.Split('\')[1]
  if ($AccountNameOnly){
   Write-Host $Name
...
function Find-BadPrivilegesDomain{
 ...
 Find-BadPrivileges -Computer $_.Name
 Write-Host " "
 }
